...
pub struct ThreadVisualizationSettings {
    pub flamegraph_collapse: bool,
    pub flamegraph_show: bool,
}
...
pub struct Options {
    ...
    /// How much we have panned vertically:
    pub pan_y_in_points: f32,
    ...
    #[cfg_attr(feature = "serde", serde(skip))]
    pub zoom_to_relative_ns_range: Option<(f64, (NanoSecond, NanoSecond))>,
}
...
pub fn ui(
    ...
    Frame::dark_canvas(ui.style()).show(ui, |ui| {
        ...
        let info = Info {
            ...
            canvas,
            response,
            painter: ui.painter().with_clip_rect(canvas),
        ...
        total_content_height = max_y - info.canvas.top();

        let timeline = paint_timeline(&info, canvas, options, min_ns);
        info.painter
            .set(where_to_put_timeline, Shape::Vec(timeline));
    });

    // Controls for Flamegraph
    ui.horizontal(|ui| {
        let (min_ns, max_ns) = if options.merge_scopes {
            frames.merged_range_ns
        } else {
        ...
        let full_range_ns = (max_ns - min_ns) as f32;
        
        ui.label("Zoom Time:");
        ui.add(egui::Slider::new(&mut options.canvas_width_ns, 1000.0..=full_range_ns).logarithmic(true).show_value(false));

        // Vertical Scroll (Depth)
        if total_content_height > canvas_rect.height() {
            let min_pan = canvas_rect.height() - total_content_height;
            ui.separator();
            ui.label("Scroll Depth:");
            // Pan Y is usually negative to scroll down
            ui.add(egui::Slider::new(&mut options.pan_y_in_points, min_pan..=0.0).show_value(false));
        }
    });
}
...
fn ui_canvas(
    ...
    let mut cursor_y = info.canvas.top() + options.pan_y_in_points;
...
fn interact_with_canvas(options: &mut Options, response: &Response, info: &Info<'_>) {
    ...
    if response.dragged_by(PointerButton::Middle) {
        ...
        options.pan_y_in_points += response.drag_delta().y;
        options.zoom_to_relative_ns_range = None;
    }

    // Zoom: Scroll Wheel
    if response.hovered() {
        let scroll_delta = info.ctx.input(|i| i.raw_scroll_delta);
        // Use Y scroll for zooming
        let zoom_delta = scroll_delta.y;
        
        if zoom_delta != 0.0 {
            // Factor: positive = zoom in, negative = zoom out
            let zoom_factor = (zoom_delta * 0.001).exp();
    ...
    if let Some((start_time, (start_ns, end_ns))) = options.zoom_to_relative_ns_range {
        ...
        // Reset Y pan on double click reset
        options.pan_y_in_points = lerp(options.pan_y_in_points..=0.0, t);

        if t >= 1.0 {
            options.zoom_to_relative_ns_range = None;
        }

        info.ctx.request_repaint_after(std::time::Duration::from_secs_f32((ZOOM_DURATION * (1.0 - t)).max(0.0).min(1.0 / 30.0)));
    }
}
