...
/// Manually yield the current frame data and start a new one.
///
/// This is useful for long-running tasks to show progress in the profiler.
/// It will close all currently open scopes, report the frame, and immediately re-open them.
pub fn yield_now() {
    ThreadProfiler::call(|tp| tp.yield_frame());
}
