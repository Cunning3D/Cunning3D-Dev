...
impl PlatformWindow for WindowsWindow {
    ...
    fn set_always_on_top(&self, on_top: bool) {
        let hwnd = self.0.hwnd;
        let insert_after = Some(if on_top { HWND_TOPMOST } else { HWND_NOTOPMOST });
        self.0.executor.spawn(async move {
            unsafe { SetWindowPos(hwnd, insert_after, 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE | SWP_NOACTIVATE).log_err(); }
        }).detach();
    }
}
