    ...
    T: HttpClientExt + Clone + 'static,
{
    ...
    async fn completion(
        ...
        if response.status().is_success() {
            ...
            tracing::info!("Received raw response from Gemini API: {}", response_text);
...
pub(crate) fn create_request_body(
    ...
    let system_instruction = completion_request.preamble.clone().map(|preamble| Content {
        // Treat preamble as a raw text part without forcing a specific role.
        // For system_instruction, Gemini REST API expects a Content object; leaving role unset
        // avoids potential incompatibilities while still sending the full preamble.
...
impl TryFrom<GenerateContentResponse> for completion::CompletionResponse<GenerateContentResponse> {
    ...
    fn try_from(response: GenerateContentResponse) -> Result<Self, Self::Error> {
                ...
                Ok(match part {
                    ...
                    other => {
                        let serialized = serde_json::to_string(other)
                            .unwrap_or_else(|_| "<unsupported gemini part>".to_string());
                        completion::AssistantContent::text(&serialized)
                    }
                })
            })
            .collect::<Result<Vec<_>, _>>()?;
        ...
        let choice = if content.is_empty() {
            if let Some(text) = response.get_text_response() {
                OneOrMany::one(completion::AssistantContent::text(&text))
            } else {
                let raw = serde_json::to_string(&response)
                    .unwrap_or_else(|_| "Gemini response contained no usable message or tool call".to_string());
                OneOrMany::one(completion::AssistantContent::text(&raw))
            }
        } else {
