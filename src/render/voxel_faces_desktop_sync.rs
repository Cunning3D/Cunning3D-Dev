...
pub fn sync_voxel_faces_root_from_final_geo_system(
    ...
    // Primary path for editor interaction: always drive voxel preview from the active voxel-edit target (selection).
    // This makes desktop behave like player: voxel preview is always available while editing, independent of display node cook timing.
    if let Some(target) = crate::coverlay_bevy_ui::resolve_voxel_edit_target(&ui_state, g) {
        ...
        let keyed = match target {
            crate::coverlay_bevy_ui::VoxelEditTarget::Direct(node_id) => {
                cunning_kernel::nodes::voxel::voxel_edit::voxel_render_sync_cmds(node_id, vs, &cmds);
                node_id
            }
            crate::coverlay_bevy_ui::VoxelEditTarget::Cda { inst_id, internal_id } => {
                cunning_kernel::nodes::voxel::voxel_edit::voxel_render_sync_cmds_for_instance(
                    inst_id,
                    internal_id,
                    vs,
                    &cmds,
    ...
    // Fallback path: if no voxel-edit is active, show voxel preview only when the final geometry is tagged as pure voxel.
