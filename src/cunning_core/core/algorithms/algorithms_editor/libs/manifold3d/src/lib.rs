...
mod ffi {
    ...
    unsafe extern "C++" {
        ...
        /// Get the face IDs of the mesh.
        fn face_ids(self: &Mesh) -> UniquePtr<CxxVector<u32>>;
        ...
        /// `face_ids` is an optional per-triangle source ID buffer; it may be empty.
        fn mesh_from_vertices(
            vertices: &[f32],
            indices: &[u32],
            face_ids: &[u32],
        ) -> UniquePtr<Mesh>;
...
impl Mesh {
    ...
    pub fn new(vertices: &[f32], indices: &[u32], face_ids: Option<&[u32]>) -> Self {
        let face_ids = face_ids.unwrap_or(&[]);
        let mesh = ffi::mesh_from_vertices(vertices, indices, face_ids);
        Self(mesh)
    }
    ...
    pub fn face_ids(&self) -> Vec<u32> {
        let face_ids_binding = self.0.face_ids();
        let face_ids = face_ids_binding.as_ref().unwrap().as_slice();
        face_ids.to_vec()
    }

    pub fn run_original_id(&self) -> Vec<u32> {
        let binding = self.0.run_original_id();
        let slice = binding.as_ref().unwrap().as_slice();
        slice.to_vec()
    }

    pub fn run_index(&self) -> Vec<u32> {
        let binding = self.0.run_index();
        let slice = binding.as_ref().unwrap().as_slice();
        slice.to_vec()
    }
