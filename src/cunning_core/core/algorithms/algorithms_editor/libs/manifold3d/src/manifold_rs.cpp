...
namespace manifold_rs
{
    ...
    std::unique_ptr<std::vector<uint32_t>> Mesh::face_ids() const
    {
        return std::make_unique<std::vector<uint32_t>>(mesh->faceID);
    }

     std::unique_ptr<std::vector<uint32_t>> Mesh::run_original_id() const
    {
        return std::make_unique<std::vector<uint32_t>>(mesh->runOriginalID);
    }

    std::unique_ptr<std::vector<uint32_t>> Mesh::run_index() const
    {
        return std::make_unique<std::vector<uint32_t>>(mesh->runIndex);
    }
    ...
    std::unique_ptr<Mesh> mesh_from_vertices(
        ...
        rust::Slice<const uint32_t> face_ids)
    {
        // Indices must always represent triangles.
        assert(indices.size() % 3 == 0);

        ::manifold::Mesh mesh;
        mesh.triVerts = std::vector<uint32_t>(indices.begin(), indices.end());

        // Deduce vertex count from the maximum index and infer the number of
        // per-vertex properties from the vertex buffer length. This allows
        // callers to provide additional properties beyond XYZ (e.g. source IDs)
        // without constraining them to a fixed stride of 3.
        uint32_t max_index = 0;
        for (auto idx : mesh.triVerts)
        {
            if (idx > max_index)
            {
                max_index = idx;
        ...
        const size_t vert_count = static_cast<size_t>(max_index) + 1;
        assert(vert_count > 0);
        assert(vertices.size() % vert_count == 0);

        const uint32_t num_prop = static_cast<uint32_t>(vertices.size() / vert_count);
        mesh.numProp = num_prop;
        mesh.vertProperties = std::vector<float>(vertices.begin(), vertices.end());

        if (!face_ids.empty()) {
            // Ensure face_ids size matches the number of triangles (indices.size() / 3)
            assert(face_ids.size() == indices.size() / 3);
            // Use pointer arithmetic for safest vector initialization from rust::Slice
            const uint32_t* src_ptr = face_ids.data();
            mesh.faceID.assign(src_ptr, src_ptr + face_ids.size());
        }
