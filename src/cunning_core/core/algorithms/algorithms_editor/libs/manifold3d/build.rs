...
"#,
    ...
    // Ensure a fresh CMake build directory for glm to avoid generator mismatch
    let glm_build_dir = std::path::Path::new(&out_dir).join("build").join("glm");
    if glm_build_dir.exists() {
        // Ignore errors here; if deletion fails, CMake will still attempt to configure
        let _ = fs::remove_dir_all(&glm_build_dir);
    }
    ...
    let manifold_build_dir = std::path::Path::new(&out_dir).join("build").join("manifold");
    if manifold_build_dir.exists() {
        let _ = fs::remove_dir_all(&manifold_build_dir);
    }
