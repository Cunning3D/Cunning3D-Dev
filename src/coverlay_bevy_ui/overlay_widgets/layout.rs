...
use super::style::OverlayTheme;
use bevy_egui::egui::{self, Ui};
use super::style::{paint_gpu_text, paint_sdf_rect};
...
/// Thin separator for toolbars (between buttons).
#[inline]
...
pub fn grid(ui: &mut Ui, cols: usize, f: impl FnOnce(&mut Ui)) {
    let theme = OverlayTheme::from_ui(ui);
    egui::Grid::new(ui.make_persistent_id("grid")).num_columns(cols).spacing(egui::vec2(4.0 * theme.scale, 4.0 * theme.scale)).show(ui, |ui| f(ui));
}
...
/// Segmented tab strip (flat section selector).
#[inline]
...
/// Flat tab strip (no pills), selected tab shows underline.
#[inline]
...
/// Compact pager: `◀ Page x/y ▶`. Returns `true` if page changed.
#[inline]
