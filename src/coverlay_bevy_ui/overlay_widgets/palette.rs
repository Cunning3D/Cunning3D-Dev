...
/// Palette strip: vertical columns from top to bottom.
#[inline]
pub fn palette_strip(ui: &mut Ui, selected: &mut u8, mut color: impl FnMut(u8) -> Color32) {
    ...
    let want = theme.palette_strip_cell_px.max(4.0);
    let cols_fit = ((avail_w / want).floor() as i32).clamp(theme.palette_strip_cols.max(1) as i32, 12) as u8;
    let cols = cols_fit.max(1);
    let rows = (((256usize + cols as usize - 1) / cols as usize).max(1)) as u8;
    // Fill width (no blank UI): cell width adapts slightly, but stays close to `want`.
    let cell_w = (avail_w / (cols as f32)).max(2.0);
    let cell_h = avail_h / (rows as f32);
    let (rect, resp) = ui.allocate_exact_size(egui::vec2(avail_w, avail_h), Sense::click());
    ...
    for x in 0..cols {
        let x0 = rect.min.x + cell_w * (x as f32);
        for y in 0..rows {
            let idx = (x as u16) * (rows as u16) + (y as u16);
            if idx == 0 || idx > 255 { continue; }
            let c = color(idx as u8);
            let y0 = rect.min.y + cell_h * (y as f32);
            let y1 = rect.min.y + cell_h * ((y + 1) as f32);
            let mut r = egui::Rect::from_min_max(egui::pos2(x0, y0), egui::pos2(x0 + cell_w, y1));
            if gap > 0.0 { r = r.shrink(gap * 0.5); }
            let sel = (idx as u8) == *selected;
            let (bc, bw) = if sel { (theme.accent, 2.0 * s) } else { (grid_c, grid_w) };
            paint_sdf_rect(ui, r, egui::CornerRadius::ZERO, c, bc, bw);
        }
    }
    if resp.clicked() {
        if let Some(p) = resp.interact_pointer_pos() {
            let x = ((p.x - rect.min.x) / cell_w.max(1e-3)).floor() as u8;
            let y = ((p.y - rect.min.y) / cell_h.max(1e-3)).floor() as u8;
            let idx = (x.min(cols - 1) as u16) * (rows as u16) + (y.min(rows - 1) as u16);
            if idx != 0 && idx <= 255 { *selected = idx as u8; }
        }
    }
}
...
pub fn color_picker_mini(ui: &mut Ui, col: &mut Color32) {
    ...
    ui.horizontal(|ui| {
        ...
        ui.vertical(|ui| {
            ui.horizontal(|ui| {
                ui.label(egui::RichText::new("R").color(theme.fg_secondary).size(10.0 * s));
                ui.add(egui::DragValue::new(&mut rgba[0]).speed(1).range(0..=255));
            });
            ui.horizontal(|ui| {
                ui.label(egui::RichText::new("G").color(theme.fg_secondary).size(10.0 * s));
                ui.add(egui::DragValue::new(&mut rgba[1]).speed(1).range(0..=255));
            });
            ui.horizontal(|ui| {
                ui.label(egui::RichText::new("B").color(theme.fg_secondary).size(10.0 * s));
                ui.add(egui::DragValue::new(&mut rgba[2]).speed(1).range(0..=255));
            });
        });
    });
...
pub fn hsv_palette_bar(ui: &mut Ui, id_source: impl std::hash::Hash, selected: &mut u8, mut color: impl FnMut(u8) -> Color32) {
    ...
    // If selection changed externally, re-seed HSV from it (cheap heuristic).
    let cur_hsv = rgb_to_hsv(color(*selected));
    if (cur_hsv.0 - hsv.0).abs() > 0.35 || (cur_hsv.1 - hsv.1).abs() > 0.35 || (cur_hsv.2 - hsv.2).abs() > 0.35 {
        hsv = cur_hsv;
    }
    ...
    let pick = |ui: &mut Ui, label: &str, _rect: egui::Rect, t: &mut f32, draw: &dyn Fn(&mut Ui, egui::Rect)| {
        ...
        if resp.dragged() || resp.clicked() {
            if let Some(p) = resp.interact_pointer_pos() {
                *t = ((p.x - bar_rect.min.x) / bar_rect.width()).clamp(0.0, 1.0);
            }
        }
    ...
    ui.vertical(|ui| {
        ...
        pick(ui, "H", ui.max_rect(), &mut hsv.0, &|ui, rect| {
            grad_bar(ui, rect, |t| { let (r, g, b) = hsv_to_rgb(t, 1.0, 1.0); Color32::from_rgb(r, g, b) });
        });
        pick(ui, "S", ui.max_rect(), &mut hsv.1, &|ui, rect| {
            let h = hsv.0;
            grad_bar(ui, rect, |t| { let (r, g, b) = hsv_to_rgb(h, t, hsv.2); Color32::from_rgb(r, g, b) });
        });
        pick(ui, "V", ui.max_rect(), &mut hsv.2, &|ui, rect| {
            let h = hsv.0;
            let s = hsv.1;
            grad_bar(ui, rect, |t| { let (r, g, b) = hsv_to_rgb(h, s, t); Color32::from_rgb(r, g, b) });
        });
    });

    // Snap to nearest palette index (deterministic palette).
