...
#[derive(Default)]
pub struct PbrMaterialNode;

impl NodeParameters for PbrMaterialNode {
    fn define_parameters() -> Vec<Parameter> {
        vec![
            ...
            Parameter::new(
                ...
                "Emissive",
                "Params",
                ParameterValue::Color(Vec3::ZERO),
...
impl NodeOp for PbrMaterialNode {
    fn compute(&self, params: &[Parameter], inputs: &[Arc<dyn GeometryRef>]) -> Arc<Geometry> {
        ...
        let use_orm = get_param_bool(params, "use_orm", true);
        let orm_tex = get_param_string(params, "orm_tex");
        let basecolor_tex = get_param_string(params, "basecolor_tex");
        let normal_tex = get_param_string(params, "normal_tex");
        let emissive_tex = get_param_string(params, "emissive_tex");
        let tint = get_param_color4(params, "tint", Vec4::ONE);
        let roughness = get_param_float(params, "roughness", 1.0);
        let metallic = get_param_float(params, "metallic", 0.0);
        let emissive = get_param_color3(params, "emissive", Vec3::ZERO);

        // Non-strict: allow partial texture sets (e.g. basecolor only). Missing textures are simply ignored by renderer.

        let mut h = std::collections::hash_map::DefaultHasher::new();
        "pbr".hash(&mut h);
        (use_orm as u8).hash(&mut h);
        orm_tex.hash(&mut h);
        basecolor_tex.hash(&mut h);
        normal_tex.hash(&mut h);
        emissive_tex.hash(&mut h);
        for b in tint.to_array().map(f32::to_bits) {
            b.hash(&mut h);
        }
        for b in emissive.to_array().map(f32::to_bits) {
            b.hash(&mut h);
        }
        ...
        geo.insert_detail_attribute(attrs::MAT_KIND, Attribute::new(vec!["pbr".to_string()]));
        ...
        geo.insert_detail_attribute(attrs::MAT_ORM_TEX, Attribute::new(vec![orm_tex]));
        ...
        geo.insert_detail_attribute(attrs::MAT_EMISSIVE, Attribute::new(vec![emissive]));
        geo.insert_detail_attribute(attrs::MAT_EMISSIVE_TEX, Attribute::new(vec![emissive_tex]));
...
#[inline]
fn get_param_color3(params: &[Parameter], name: &str, default: Vec3) -> Vec3 {
    params
        .iter()
        .find(|p| p.name == name)
        .and_then(|p| match &p.value {
...
register_node!("PBR Material", "Material", PbrMaterialNode);
