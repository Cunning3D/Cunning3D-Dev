...
## Cunning3D fork (cunning_egui)

This repository contains a **Cunning3D-customized fork** of upstream `egui` **0.27.2**, internally referred to as `cunning_egui`.

本仓库中的 `egui` 是基于官方 `0.27.2` 的 **Cunning3D 专用魔改分支**，后续与上游可能继续产生差异。

与官方 `egui 0.27.2` 的主要差异：

- **参数梯度 Ladder（中键阶梯滑条）**
  - 为 `widgets::DragValue` 增加中键触发的纵向阶梯菜单（`widgets::ladder` 模块）。
  - 使用固定颗粒度档位：
    - 浮点：`100, 10, 1, .1, .01, .001, .0001`
    - 整数：`100, 10, 1`
  - 中键按住：
    - 光标在阶梯列内垂直移动：只切换当前选中档位（橙色高亮），**不直接改值**。
    - 水平移动：按当前档位对参数进行精细/粗略调节。
- **DragValue 参数交互增强（面向 DCC 工作流）**
  - 移除了原版 `DragValue` 的左键拖拽改值行为，避免 hover 即可拖动的“滑块式”交互。
  - 当前仅保留两种改值方式：中键 Ladder 拖拽（分档颗粒度）和左键点击进入文本编辑手动输入。
  - `widgets::drag_value` 的交互逻辑根据 Cunning3D 的节点属性面板需求做了针对性调整。

以上改动仅作用于本仓库内置的 `egui`，与上游官方 crate API 不完全兼容；在升级上游版本或复用该 fork 时需注意这些差异。
