...
use crate::{
    ...
    TextEdit, TextWrapMode, Ui, Widget, WidgetInfo, emath, text, Align2, Color32, FontId,
    LayerId, Order, PointerButton, Pos2, Rect, Stroke, StrokeKind,
};
    ...
    fn ui(self, ui: &mut Ui) -> Response {
        ...
        let mut response = if is_kb_editing {
            ...
            if response.clicked() {
                ...
                select_all_text(ui, id, response.id, &value_text);
            } else if response.is_pointer_button_down_on() && ui.input(|i| i.pointer.button_down(PointerButton::Middle))
                || response.drag_started_by(PointerButton::Middle)
                || response.dragged_by(PointerButton::Middle)
            {
                // Houdini-style MMB ladder: vertical=档位, horizontal=改值
                let sid = id.with("__mmb_scrub");
                let mut st = ui.data(|d| d.get_temp::<MmbScrubState>(sid).unwrap_or_default());

                if !st.active {
                    st.active = true;
                    st.tier = 0;
                    st.start_pos = ui.ctx().pointer_interact_pos().unwrap_or_else(|| response.rect.center());
                }

                if st.active && response.dragged_by(PointerButton::Middle) {
                    // --- 档位计算 ---
                    let band_h = 22.0_f32;
                    let total_y = response.total_drag_delta().map(|d| d.y).unwrap_or(0.0);
                    st.tier = (total_y / band_h).round().clamp(-3.0, 3.0) as i32;

                    let mul = 10.0_f64.powi(st.tier);
                    let eff_speed = (if is_slow_speed { speed / 10.0 } else { speed }) * mul;

                    // --- 改值 ---
                    let motion = response.drag_motion();
                    let delta_value = motion.x as f64 * eff_speed;
                    if delta_value != 0.0 {
                        let precise = ui.data_mut(|data| data.get_temp::<f64>(id)).unwrap_or(value);
                        let precise = precise + delta_value;
                        let aim_delta = aim_rad * eff_speed.abs();
                        let rounded = emath::smart_aim::best_in_range_f64(precise - aim_delta, precise + aim_delta);
                        let rounded = emath::round_to_decimals(rounded, auto_decimals);
                        let rounded = clamp_value_to_range(rounded, range.clone());
                        set(&mut get_set_value, rounded);
                        ui.data_mut(|data| data.insert_temp::<f64>(id, precise));
                        ui.data_mut(|data| data.remove::<String>(id));
                    }

                    // --- Houdini-native Ladder (极简) ---
                    let layer = LayerId::new(Order::Foreground, sid);
                    let p = ui.ctx().layer_painter(layer);

                    let row_h = 18.0_f32;
                    let ind_w = 3.0_f32;
                    let text_w = 42.0_f32;
                    let panel_w = ind_w + text_w + 4.0;
                    let panel_h = row_h * 7.0;

                    let panel_pos = st.start_pos + emath::vec2(12.0, -panel_h * 0.5);
                    let panel_rect = Rect::from_min_size(panel_pos, emath::vec2(panel_w, panel_h));

                    // 纯深色背景
                    p.rect_filled(panel_rect, 2.0, Color32::from_gray(28));
                    p.rect_stroke(panel_rect, 2.0, Stroke::new(1.0, Color32::from_gray(50)), StrokeKind::Inside);

                    let accent = ui.visuals().selection.bg_fill;
                    let font = FontId::proportional(11.0);
                    let tiers: [i32; 7] = [-3, -2, -1, 0, 1, 2, 3];

                    for (idx, &tier_val) in tiers.iter().enumerate() {
                        let y0 = panel_rect.top() + row_h * idx as f32;
                        let row_rect = Rect::from_min_size(Pos2::new(panel_rect.left(), y0), emath::vec2(panel_w, row_h));
                        let is_active = tier_val == st.tier;

                        // 左侧指示条
                        let ind_rect = Rect::from_min_size(row_rect.left_top(), emath::vec2(ind_w, row_h));
                        if is_active {
                            p.rect_filled(ind_rect, 0.0, accent);
                        }

                        // 档位文字
                        let m = 10.0_f64.powi(tier_val);
                        let txt = if m >= 1.0 { format!("{:.0}", m) } else if m >= 0.01 { format!("{:.2}", m) } else { format!("{:.3}", m) };
                        let col = if is_active { Color32::WHITE } else { Color32::from_gray(140) };
                        p.text(Pos2::new(panel_rect.left() + ind_w + 4.0, row_rect.center().y), Align2::LEFT_CENTER, txt, font.clone(), col);
                    }

                    ui.data_mut(|d| d.insert_temp(sid, st));
                    ui.ctx().request_repaint();
                }

                if ui.input(|i| i.pointer.button_released(PointerButton::Middle)) {
                    ui.data_mut(|d| d.remove::<MmbScrubState>(sid));
                }
