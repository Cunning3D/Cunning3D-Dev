...
pub fn best_in_range_f64(min: f64, max: f64) -> f64 {
    ...
    for i in 0..NUM_DECIMALS {
        if min_str[i] == max_str[i] {
            ...
            // NOTE: Extremely close min/max can, after scaling and rounding, produce
            // inverted digit strings (e.g. min_str[i] >= max_str[i]) due to float
            // precision/rounding. This should never crash the app.
            if deciding_digit_min >= deciding_digit_max {
                return fast_midpoint(min, max);
            }
            ...
            let deciding_digit = if deciding_digit_min == 0 {
                ...
                (deciding_digit_min + deciding_digit_max) / 2
            };
