...
#[cfg(feature = "render")]
pub mod egui_node;
/// Egui hole punching for underlying UI.
#[cfg(feature = "holes")]
pub mod holes;
...
pub use egui;
#[cfg(feature = "holes")]
pub use holes::{EguiHole, EguiHoleDisable, EguiHolesMode, EguiHolesPlugin, EguiHolesSettings};
...
use bevy::{
    ...
    prelude::{
        Added, Commands, Component, Deref, DerefMut, Entity, Query, Resource,
...
impl Plugin for EguiPlugin {
    fn build(&self, app: &mut App) {
        ...
        world.init_resource::<EguiOcclusionRects>();
        #[cfg(feature = "render")]
        ...
        #[cfg(feature = "render")]
        app.add_plugins(ExtractResourcePlugin::<EguiOcclusionRects>::default());
        #[cfg(feature = "render")]
        ...
        #[cfg(feature = "holes")]
        app.add_plugins(holes::EguiHolesPlugin);
        ...
        app.add_systems(Last, free_egui_textures_system)
            .add_systems(
                Render,
                render_systems::prepare_egui_transforms_system.in_set(RenderSystems::PrepareResources),
            )
            .add_systems(
                Render,
                render_systems::queue_bind_groups_system.in_set(RenderSystems::PrepareBindGroups),
            )
            .add_systems(
                Render,
                render_systems::queue_pipelines_system.in_set(RenderSystems::Queue),
            );
    ...
    fn finish(&self, app: &mut App) {
        ...
        if let Some(render_app) = app.get_sub_app_mut(RenderApp) {
            render_app
                ...
                .init_resource::<render_systems::EguiNodes>()
                .add_systems(
                    ExtractSchedule,
                    render_systems::extract_egui_window_ui_system,
                )
                .add_systems(
                    Render,
                    render_systems::prepare_egui_transforms_system.in_set(RenderSystems::PrepareResources),
                ...
                .add_systems(
                    ...
                    render_systems::render_egui_overlay_system.after(bevy_core_pipeline::schedule::camera_driver),
                );
        }
    }
}
...
fn free_egui_textures_system(
    ...
    for image_event in image_events.read() {
        if let AssetEvent::Removed { id } = image_event {
            egui_user_textures.textures.retain(|h, _| h.id() != *id);
        }
    }
}
