...
use egui::{
    ...
    LayerId, Layout, NumExt, Order, Rect, Response, CornerRadius, Popup, ScrollArea, Sense, Stroke,
    TextStyle, Ui, UiBuilder, UiKind, Vec2, WidgetText,
};
...
impl<'tree, Tab> DockArea<'tree, Tab> {
    ...
    fn tab_bar(
        ...
        if style.tab_bar.hide_single_tab {
            let tabs_len = self.dock_state[surface_index][node_index]
                .tabs()
                .map(|t| t.len())
                .unwrap_or(0);
            if tabs_len <= 1 {
    ...
    fn tab_plus(
        ...
        // REMOVED: Left border line for cleaner look
        /*
        ui.painter().vline(
            rect.left(),
            rect.y_range(),
    ...
    fn tab_title(
        ...
        // --- Houdini Style Tabs ---
        if active {
            // Active Tab: Subtle background + Bottom Highlight
            ui.painter().rect_filled(tab_rect, tab_style.rounding, tab_style.bg_fill);
            
            // Bottom Highlight Line
            let stroke_width = 2.0;
            let y = tab_rect.bottom() - stroke_width / 2.0;
             ui.painter().line_segment(
                [pos2(tab_rect.min.x, y), pos2(tab_rect.max.x, y)],
                Stroke::new(stroke_width, tab_style.text_color) // Use text color (usually white/bright) for highlight
            );
        } else if response.hovered() {
             // Hovered Tab: Very subtle background
             ui.painter().rect_filled(tab_rect, tab_style.rounding, tab_style.bg_fill.linear_multiply(0.5));
        }
    ...
    fn tab_body(
        ...
        if let Some(tab) = tabs.get_mut(active.0) {
            *viewport = body_rect;
            ...
            // from node to node.
            //
            // FIX: Include surface and node index to allow same tab in multiple nodes
            let id = self.id.with((surface_index, node_index, tab_viewer.id(tab)));
