use egui::{
    CollapsingHeader, CollapsingResponse, Color32, Frame, Galley, Id, Layout, Rect, Response, Sense,
...
impl<'tree, Tab> DockArea<'tree, Tab> {
    pub(super) fn show_window_surface(
        ...
        // Get galley of currently selected node as a window title (and whether this window must stay transparent).
        let (title, wants_transparent_frame) = {
            ...
            let wants_transparent_frame = !tab_viewer.clear_background(&tabs[active.0]);
            (title, wants_transparent_frame)
        };

        // Fade window frame (if necessary)
        let mut frame = Frame::window(ui.style());
        if wants_transparent_frame {
            frame.fill = Color32::TRANSPARENT;
            frame.stroke.color = Color32::TRANSPARENT;
            frame.shadow.color = Color32::TRANSPARENT;
        }
        if fade_factor != 1.0 {
...
fn close_button(disabled: Option<&str>) -> impl Widget + '_ {
    move |ui: &mut Ui| -> Response {
        ...
        painter.line_segment(
            [
                rect.left_top().round(),
                rect.right_bottom().round(),
            ],
            stroke,
        );
        painter.line_segment(
            [
                rect.right_top().round(),
                rect.left_bottom().round(),
            ],
            stroke,
        );
        match disabled {
