...
fn button_ui(
    ...
    let painter = make_overlay_painter(ui);
    painter.rect_stroke(rect, 0.0, visuals.button_border_stroke, egui::StrokeKind::Inside);
    let rect = rect.shrink(rect.width() * 0.1);
    painter.rect_stroke(rect, 0.0, button_stroke, egui::StrokeKind::Inside);
    let rim = { Rect::from_two_pos(rect.min, rect.lerp_inside(vec2(1.0, 0.1))) };
    painter.rect(rim, 0.0, visuals.button_color, Stroke::NONE, egui::StrokeKind::Inside);
...
impl DragDropState {
    ...
    pub(super) fn resolve_icon_based(
        ...
        let mut destination: Option<TabDestination> = None;

        let center = rect.center();

        // 原始中心正方形：保持与旧版相同的整体区域。
        let center_rect = Rect::from_center_size(center, Vec2::splat(shortest_side));

        // 在中心正方形内部再垂直划分为上下两块：
        // - 上：保持原有行为（插入当前节点/空 surface）。
        // - 下：代表“浮动到新窗口”。
        let mid_y = center_rect.center().y;
        let center_top = Rect::from_min_max(
            Pos2::new(center_rect.min.x, center_rect.min.y),
            Pos2::new(center_rect.max.x, mid_y),
        );
        let center_bottom = Rect::from_min_max(
            Pos2::new(center_rect.min.x, mid_y),
            Pos2::new(center_rect.max.x, center_rect.max.y),
        );

        // 上半：当前节点 / 空 surface 内部的 tab stack。
        if button_ui(center_top, ui, &mut hovering_buttons, pointer, style, None) {
        ...
        // 下半：仅当允许窗口时才出现，表示“浮动为新窗口”。
        if windows_allowed
            && button_ui(center_bottom, ui, &mut hovering_buttons, pointer, style, None)
        {
    ...
    pub(super) fn is_locked(&self, style: &Style, ctx: &Context) -> bool {
        match self.locked.as_ref() {
            Some(lock_time) => {
                ...
                let rem = style.overlay.feel.max_preference_time - elapsed;
                if rem > 0.0 {
