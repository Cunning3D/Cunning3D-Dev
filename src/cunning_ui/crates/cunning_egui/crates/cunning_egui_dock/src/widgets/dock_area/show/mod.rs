use egui::{
    CentralPanel, Color32, Context, CursorIcon, EventFilter, Frame, Key, Pos2, Rect, CornerRadius,
    Sense, Ui, Vec2,
};
...
impl<'tree, Tab> DockArea<'tree, Tab> {
    ...
    pub fn show_inside(mut self, ui: &mut Ui, tab_viewer: &mut impl TabViewer<Tab = Tab>) {
        ...
        if let (Some(source), Some(hover)) = (drag_data, hover_data) {
            ...
            let mut tab_dst = self.show_drag_drop_overlay(ui, &mut state, tab_viewer);

            if pointer_released {
                // If the user releases the mouse while dragging a tab and the pointer is
                // outside the main window (no hover_pos) AND no regular drop destination
                // was resolved, interpret this as "detach to window" (like IDEs).
                if tab_dst.is_none() && pointer_pos.is_none() {
                    if let Some(dnd_state) = &state.dnd {
                        // Recompute whether this tab is allowed to be shown in its own window.
                        let windows_allowed = match dnd_state.drag.src {
                            TreeComponent::Tab(surface, node, tab) => {
                                let Node::Leaf { tabs, .. } =
                                    &mut self.dock_state[surface][node]
                                else {
                        ...
                        if windows_allowed {
                            let pointer = dnd_state.pointer;
                            let rect =
                                Rect::from_min_size(pointer, dnd_state.drag.rect.size());
                            tab_dst = Some(TabDestination::Window(rect));
                        }
                    }
                }
                ...
                if let Some(destination) = tab_dst {
                    ...
                    match destination {
                        TabDestination::Window(rect) => {
                            if let Some(handler) = self.on_tab_window.as_mut() {
                                handler(self.dock_state, source, rect);
                            } else {
                                self.dock_state.move_tab(source, TabDestination::Window(rect));
                            }
                        }
                        other => {
                            self.dock_state.move_tab(source, other);
                        }
                    }
                }
            }
    ...
    fn hovered_window_surface(
        ...
        state.window_fade.and_then(|(time, surface)| {
            let elapsed = ctx.input(|i| (i.time - time) as f32);
            let rem = hold_time - elapsed;
            if rem > 0.0 {
                ctx.request_repaint_after(std::time::Duration::from_secs_f32(rem.min(1.0 / 30.0)));
                Some(surface)
            } else {
