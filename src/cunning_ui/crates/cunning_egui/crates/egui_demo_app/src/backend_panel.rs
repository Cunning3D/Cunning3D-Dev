...
impl BackendPanel {
    ...
    pub fn ui(&mut self, ui: &mut egui::Ui, frame: &mut eframe::Frame) {
        ...
        {
            ui.separator();
            ui.collapsing("GPU Text settings", |ui| {
                let mut t = egui_wgpu::sdf::gpu_text_tuning_get();
                let mut changed = false;
                ui.horizontal(|ui| {
                    changed |= ui.add(egui::Slider::new(&mut t.scale, 0.5..=1.5).text("scale")).changed();
                    changed |= ui.add(egui::Slider::new(&mut t.line_height_factor, 1.0..=2.0).text("line height")).changed();
                });
                ui.horizontal(|ui| {
                    changed |= ui.add(egui::Slider::new(&mut t.y_offset_points, -2.0..=2.0).text("y offset")).changed();
                    changed |= ui.add(egui::Slider::new(&mut t.scissor_pad_y, 0..=16).text("scissor pad y(px)")).changed();
                });
                ui.horizontal(|ui| {
                    changed |= ui.add(egui::Slider::new(&mut t.gamma, 1.0..=2.2).text("gamma")).changed();
                    changed |= ui.add(egui::Slider::new(&mut t.grayscale_enhanced_contrast, 0.0..=2.0).text("contrast")).changed();
                });
                ui.horizontal(|ui| {
                    ui.label("font");
                    changed |= egui::ComboBox::from_id_source("gpu_text_font_demo")
                        .selected_text(match t.sans_family { 1 => "Segoe UI", 2 => "Microsoft YaHei", 3 => "SimHei", 4 => "SimSun", _ => "Auto" })
                        .show_ui(ui, |ui| {
                            ui.selectable_value(&mut t.sans_family, 0, "Auto").changed()
                                || ui.selectable_value(&mut t.sans_family, 1, "Segoe UI").changed()
                                || ui.selectable_value(&mut t.sans_family, 2, "Microsoft YaHei").changed()
                                || ui.selectable_value(&mut t.sans_family, 3, "SimHei").changed()
                                || ui.selectable_value(&mut t.sans_family, 4, "SimSun").changed()
                        })
                        .inner;
                });
                ui.horizontal(|ui| {
                    changed |= ui.add(egui::Slider::new(&mut t.upload_budget, 0..=512).text("upload budget/frame")).changed();
                });
                changed |= ui.add(egui::Slider::new(&mut t.content_weight, 0.5..=2.0).text("weight (thinning)")).changed();
                changed |= ui.checkbox(&mut t.row_bounds_clamp, "row bounds clamp (avoid overlap)").changed();
                let load_sys = ui.checkbox(&mut t.load_system_fonts, "load_system_fonts (needs restart)").changed();
                changed |= load_sys;
