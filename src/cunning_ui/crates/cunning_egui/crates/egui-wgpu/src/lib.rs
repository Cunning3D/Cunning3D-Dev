...
/// SDF Rendering Utilities (CunningUI Extension)
pub mod sdf { pub use cunning_wgpu_ui::sdf::*; }
...
impl RenderState {
    ...
    pub async fn create(
        ...
        #[cfg(not(target_arch = "wasm32"))]
        let available_adapters = instance.enumerate_adapters(wgpu::Backends::all()).await;
        ...
        let (device, queue) = {
            ...
            adapter
                .request_device(&(*config.device_descriptor)(&adapter))
                .await?
        };
        ...
        let renderer = Renderer::new(
            &device,
            target_format,
            RendererOptions { msaa_samples, depth_stencil_format: depth_format, ..Default::default() },
        );
...
pub fn adapter_info_summary(info: &wgpu::AdapterInfo) -> String {
    let wgpu::AdapterInfo { name, vendor, device, device_type, driver, driver_info, backend, .. } = &info;
