...
impl Painter {
    ...
    ///
    /// Call before the first and after the last resize event. Used for platform-specific logic,
    /// e.g. the macOS Metal window resize jitter fix.
    pub fn on_window_resize_state_change(&mut self, viewport_id: ViewportId, resizing: bool) {
        crate::profile_function!();

        let Some(state) = self.surfaces.get_mut(&viewport_id) else { return; };
        if state.resizing == resizing {
            return;
        }

        // Resizing on macOS + Metal requires setting present_with_transaction to avoid jitter.
        ...
        unsafe {
            if let Some(hal_surface) = state.surface.as_hal::<wgpu::hal::api::Metal>() {
                ...
                Self::configure_surface(state, self.render_state.as_ref().unwrap(), &self.configuration);
            }
        }
    ...
    pub fn paint_and_update_textures(
        ...
        let user_cmd_bufs = {
            ...
            // Rebuild SDF font atlas at most once per frame, after applying all texture deltas.
            renderer.flush_font_sdf(&render_state.device, &render_state.queue, &mut encoder);
