...
pub struct SdfDashedCurveUniform {
    pub p01: [f32; 4],            // p0.xy, p1.xy
    pub p23: [f32; 4],            // p2.xy, p3.xy
    pub color: [f32; 4],          // rgba
    pub params: [f32; 4],         // thickness, softness, dash_len, gap_len
    pub screen_params: [f32; 4],  // screen_size.xy, dash_offset, pad
}

const SDF_DASHED_CURVE_SHADER: &str = r#"
struct VertexOutput {
...
struct Uniforms {
    ...
    p23: vec4<f32>,
    color: vec4<f32>,
    params: vec4<f32>,        // x=thickness, y=softness, z=dash_len, w=gap_len
    screen_params: vec4<f32>, // xy=screen_size, z=dash_offset
};
...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let half_thick = u.params.x * 0.5;
    let sdf = min_dist - half_thick;
    let aa = max(1.0, u.params.y);
    var alpha = 1.0 - smoothstep(-aa * 0.5, aa * 0.5, sdf);
    if (alpha <= 0.0) { discard; }

    // Dash mask in screen-space length along the polyline approximation
    let dash = max(0.5, u.params.z);
    let gap = max(0.0, u.params.w);
    let period = dash + gap;
    let phase = (best_s + u.screen_params.z) / max(1e-3, period);
    let f = fract(phase);
    let dash_mask = step(f, dash / max(1e-3, period));

    alpha = alpha * dash_mask;
    if (alpha <= 0.0) { discard; }
...
pub struct SdfDashedCurveBatchStats { pub frame_id: u64, pub instances: u64, pub clip_regions: u64, pub drawcalls: u64 }

#[derive(Clone, Copy, Debug, Default)]
pub struct SdfDashedCurveClipBatchStat { pub scissor: [u32; 4], pub instances: u32 }

static SDF_DASHED_LAST_FRAME: AtomicU64 = AtomicU64::new(0);
static SDF_DASHED_LAST_INST: AtomicU64 = AtomicU64::new(0);
static SDF_DASHED_LAST_REGIONS: AtomicU64 = AtomicU64::new(0);
static SDF_DASHED_LAST_DRAWCALLS: AtomicU64 = AtomicU64::new(0);
static SDF_DASHED_VERBOSE: std::sync::atomic::AtomicBool = std::sync::atomic::AtomicBool::new(false);
static SDF_DASHED_LAST_DETAILS_FRAME: AtomicU64 = AtomicU64::new(0);
static SDF_DASHED_LAST_DETAILS: OnceLock<Mutex<Vec<SdfDashedCurveClipBatchStat>>> = OnceLock::new();

pub fn sdf_dashed_curve_last_stats() -> SdfDashedCurveBatchStats {
    SdfDashedCurveBatchStats {
        frame_id: SDF_DASHED_LAST_FRAME.load(Ordering::Relaxed),
        instances: SDF_DASHED_LAST_INST.load(Ordering::Relaxed),
        clip_regions: SDF_DASHED_LAST_REGIONS.load(Ordering::Relaxed),
        drawcalls: SDF_DASHED_LAST_DRAWCALLS.load(Ordering::Relaxed),
    }
}

pub fn sdf_dashed_curve_set_verbose_details_enabled(enabled: bool) { SDF_DASHED_VERBOSE.store(enabled, Ordering::Relaxed); }

pub fn sdf_dashed_curve_last_batch_details() -> (u64, Vec<SdfDashedCurveClipBatchStat>) {
    let fid = SDF_DASHED_LAST_DETAILS_FRAME.load(Ordering::Relaxed);
    let v = SDF_DASHED_LAST_DETAILS.get_or_init(|| Mutex::new(Vec::new())).lock().unwrap().clone();
    (fid, v)
}

pub struct SdfDashedCurveRenderer {
    pub pipeline: wgpu::RenderPipeline,
    pub bind_group_layout: wgpu::BindGroupLayout,
}

impl SdfDashedCurveRenderer {
    pub fn new(device: &wgpu::Device, format: wgpu::TextureFormat) -> Self {
        let shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            label: Some("SDF Dashed Curve Shader"),
            source: wgpu::ShaderSource::Wgsl(SDF_DASHED_CURVE_SHADER.into()),
        });
        let bind_group_layout = device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
            label: Some("SDF Dashed Curve Bind Group Layout"),
        ...
        let pipeline_layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
            label: Some("SDF Dashed Curve Pipeline Layout"),
            bind_group_layouts: &[&bind_group_layout],
            immediate_size: 0,
        });
        let pipeline = device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
            label: Some("SDF Dashed Curve Pipeline"),
    ...
    pub fn prepare_bind_group(&self, device: &wgpu::Device, instances: &[SdfDashedCurveUniform]) -> wgpu::BindGroup {
        let buffer = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
            label: Some("SDF Dashed Curve Instances"),
            contents: bytemuck::cast_slice(instances),
            usage: wgpu::BufferUsages::STORAGE,
        });
        device.create_bind_group(&wgpu::BindGroupDescriptor {
            label: Some("SDF Dashed Curve Bind Group"),
...
struct Batch {
    scissor: [u32; 4],
    instances: Vec<SdfDashedCurveUniform>,
    bind_group: Option<wgpu::BindGroup>,
    leader: bool,
}

struct SdfDashedCurveQueue {
    pub renderer: Arc<SdfDashedCurveRenderer>,
    batches: Vec<Batch>,
...
pub struct SdfDashedCurveCallback {
    pub uniform: SdfDashedCurveUniform,
    pub frame_id: u64,
    pub _clip: egui::Rect,
...
impl crate::CallbackTrait for SdfDashedCurveCallback {
    fn prepare(
        &self,
        device: &wgpu::Device,
        _queue: &wgpu::Queue,
        ...
        let queue = resources.entry::<SdfDashedCurveQueue>().or_insert_with(|| SdfDashedCurveQueue {
            renderer: Arc::new(SdfDashedCurveRenderer::new(device, fmt)),
    ...
    fn finish_prepare(&self, device: &wgpu::Device, _queue: &wgpu::Queue, _egui_encoder: &mut wgpu::CommandEncoder, resources: &mut crate::CallbackResources) -> Vec<wgpu::CommandBuffer> {
        let fid = resources.get::<super::SdfFrameId>().map(|v| v.0).unwrap_or(self.frame_id);
        let Some(q) = resources.get_mut::<SdfDashedCurveQueue>() else { return Vec::new(); };
        ...
        SDF_DASHED_LAST_FRAME.store(fid, Ordering::Relaxed);
        SDF_DASHED_LAST_INST.store(inst_total, Ordering::Relaxed);
        SDF_DASHED_LAST_REGIONS.store(regions, Ordering::Relaxed);
        SDF_DASHED_LAST_DRAWCALLS.store(regions, Ordering::Relaxed);
        if SDF_DASHED_VERBOSE.load(Ordering::Relaxed) {
            let mut out: Vec<SdfDashedCurveClipBatchStat> = Vec::with_capacity(q.batches.len());
            for b in q.batches.iter().filter(|b| !b.instances.is_empty()) {
                out.push(SdfDashedCurveClipBatchStat { scissor: b.scissor, instances: b.instances.len() as u32 });
            }
            out.sort_by_key(|b| (b.scissor[0], b.scissor[1], b.scissor[2], b.scissor[3]));
            *SDF_DASHED_LAST_DETAILS.get_or_init(|| Mutex::new(Vec::new())).lock().unwrap() = out;
            SDF_DASHED_LAST_DETAILS_FRAME.store(fid, Ordering::Relaxed);
        }
        Vec::new()
    }

    fn paint<'a>(&'a self, info: egui::PaintCallbackInfo, render_pass: &mut wgpu::RenderPass<'a>, resources: &'a crate::CallbackResources) {
        if let Some(q) = resources.get::<SdfDashedCurveQueue>() {
...
pub fn create_sdf_dashed_curve_callback(rect: egui::Rect, uniform: SdfDashedCurveUniform, frame_id: u64) -> egui::PaintCallback {
    crate::Callback::new_paint_callback(rect, SdfDashedCurveCallback { uniform, frame_id, _clip: rect, key: Arc::new(AtomicU64::new(0)) })
}
