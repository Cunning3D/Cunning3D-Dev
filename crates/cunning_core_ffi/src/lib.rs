...
#[repr(C)]
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum JobOutputKind { GeoHandle = 1, ParamJson = 2 }
...
struct JobRuntime {
    ...
    outputs: DashMap<JobId, Vec<JobOutput>>,
...
fn worker_loop(rx: Receiver<JobRequest>) {
    while let Ok(req) = rx.recv() {
        ...
        let mut outs: Vec<JobOutput> = Vec::new();
        let ok = catch_unwind(AssertUnwindSafe(|| {
            ...
            let selected: Option<Vec<String>> = req.exports_json.as_ref().and_then(|s| serde_json::from_str::<Vec<String>>(s).ok());
            if let Some(sel) = selected.as_deref() {
                let xs = match cda_rt::execute_selected(&entry.plan, &entry.def, cda_registry(), &inputs, &params, &req.cancel, Some(sel)) {
                    Ok(v) => v,
                    Err(e) => { rt.errors.insert(req.job_id, format!("{:?}", e)); return None; }
                };
                for (_name, v) in xs {
                    match v {
                        cda_rt::ExportValue::Geometry(g) => {
                            let h = register_geo_handle(g);
                            if out0 == 0 { out0 = h; }
                            outs.push(JobOutput::Geo(h));
                        }
                        cda_rt::ExportValue::Param(p) => {
                            let json = serde_json::to_string(&p).unwrap_or_else(|_| "null".to_string());
                            outs.push(JobOutput::ParamJson(json));
                        }
                    }
                }
            } else {
...
#[no_mangle]
pub extern "C" fn cunning_cda_get_export_count(cda_id: u64) -> u32 {
    let result = catch_unwind(AssertUnwindSafe(|| {
        cda_reg().lock().unwrap().get(&cda_id).map(|e| e.def.exports.len() as u32).unwrap_or(0)
    }));
    result.unwrap_or(0)
}

#[no_mangle]
pub extern "C" fn cunning_cda_get_export_kind(cda_id: u64, index: u32) -> JobOutputKind {
    let result = catch_unwind(AssertUnwindSafe(|| {
        let e = cda_reg().lock().unwrap().get(&cda_id).cloned()?;
        let ex = e.def.exports.get(index as usize)?;
        Some(match ex.kind {
            cda_rt::asset::ExportKind::NodeOutput { .. } => JobOutputKind::GeoHandle,
            cda_rt::asset::ExportKind::NodeParam { .. } => JobOutputKind::ParamJson,
        })
    }));
    result.unwrap_or(None).unwrap_or(JobOutputKind::GeoHandle)
}

#[no_mangle]
pub extern "C" fn cunning_cda_get_export_name(cda_id: u64, index: u32, out_buf: *mut c_char, cap: u32) -> u32 {
    let result = catch_unwind(AssertUnwindSafe(|| {
        if out_buf.is_null() || cap == 0 { return None; }
        let e = cda_reg().lock().unwrap().get(&cda_id).cloned()?;
        let name = e.def.exports.get(index as usize).map(|x| x.name.as_str()).unwrap_or("");
...
pub extern "C" fn cunning_cda_submit(cda_id: u64, instance_id: InstanceId, gen: u64, params_json: *const c_char, input_handles: *const Handle, input_count: u32) -> JobId {
    let result = catch_unwind(AssertUnwindSafe(|| {
        ...
        let _ = rt.tx.send(JobRequest { job_id, instance_id, gen, cancel, cda_id, params_json: json, inputs, exports_json: None });
        Some(job_id)
    }));
    result.unwrap_or(None).unwrap_or(0)
}
...
#[no_mangle]
pub extern "C" fn cunning_cda_submit_select(cda_id: u64, instance_id: InstanceId, gen: u64, params_json: *const c_char, input_handles: *const Handle, input_count: u32, exports_json: *const c_char) -> JobId {
    let result = catch_unwind(AssertUnwindSafe(|| {
        let rt = job_rt();
        rt.latest_gen.insert(instance_id, gen);
        let job_id = NEXT_JOB_ID.fetch_add(1, Ordering::Relaxed);
        ...
        let json = if params_json.is_null() { "{}".to_string() } else { unsafe { CStr::from_ptr(params_json) }.to_str().unwrap_or("{}").to_string() };
        let inputs = if input_handles.is_null() || input_count == 0 { Vec::new() } else { unsafe { std::slice::from_raw_parts(input_handles, input_count as usize) }.to_vec() };
        let ex = if exports_json.is_null() { None } else { Some(unsafe { CStr::from_ptr(exports_json) }.to_str().unwrap_or("[]").to_string()) };

        let _ = rt.tx.send(JobRequest { job_id, instance_id, gen, cancel, cda_id, params_json: json, inputs, exports_json: ex });
        Some(job_id)
    }));
    result.unwrap_or(None).unwrap_or(0)
}
...
pub extern "C" fn cunning_job_get_output_handle(job_id: JobId, index: u32) -> Handle {
    let result = catch_unwind(AssertUnwindSafe(|| {
        let rt = job_rt();
        rt.outputs.get(&job_id).and_then(|v| v.get(index as usize)).and_then(|o| match o { JobOutput::Geo(h) => Some(*h), _ => None }).unwrap_or(0)
    }));
    result.unwrap_or(0)
}

#[no_mangle]
pub extern "C" fn cunning_job_get_output_kind(job_id: JobId, index: u32) -> JobOutputKind {
    let result = catch_unwind(AssertUnwindSafe(|| {
        let rt = job_rt();
        rt.outputs.get(&job_id).and_then(|v| v.get(index as usize)).map(|o| match o { JobOutput::Geo(_) => JobOutputKind::GeoHandle, JobOutput::ParamJson(_) => JobOutputKind::ParamJson }).unwrap_or(JobOutputKind::GeoHandle)
    }));
    result.unwrap_or(JobOutputKind::GeoHandle)
}

#[no_mangle]
pub extern "C" fn cunning_job_get_output_geo_handle(job_id: JobId, index: u32) -> Handle {
    cunning_job_get_output_handle(job_id, index)
}

#[no_mangle]
pub extern "C" fn cunning_job_get_output_param_json(job_id: JobId, index: u32, out_buf: *mut c_char, cap: u32) -> u32 {
    let result = catch_unwind(AssertUnwindSafe(|| {
        if out_buf.is_null() || cap == 0 { return None; }
        let rt = job_rt();
        let s = rt.outputs.get(&job_id).and_then(|v| v.get(index as usize)).and_then(|o| match o { JobOutput::ParamJson(s) => Some(s.as_str()), _ => None }).unwrap_or("");
...
#[repr(C)]
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum GeoAttrClass { Detail = 0, Point = 1, Vertex = 2, Primitive = 3, Edge = 4 }

#[repr(C)]
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum GeoAttrType { Unknown = 0, F32 = 1, Vec2 = 2, Vec3 = 3, Vec4 = 4, I32 = 5, IVec2 = 6, BoolU8 = 7 }

fn geo_attr<'a>(geo: &'a Geometry, class: GeoAttrClass, name: &str) -> Option<&'a Attribute> {
    match class {
        GeoAttrClass::Detail => geo.get_detail_attribute(name),
        GeoAttrClass::Point => geo.get_point_attribute(name),
        GeoAttrClass::Vertex => geo.get_vertex_attribute(name),
        GeoAttrClass::Primitive => geo.get_primitive_attribute(name),
        GeoAttrClass::Edge => geo.get_edge_attribute(name),
    }
}

fn attr_len(a: &Attribute) -> u32 { a.len().min(u32::MAX as usize) as u32 }

#[no_mangle]
pub extern "C" fn cunning_geo_get_attr_len(handle: Handle, class: GeoAttrClass, name: *const c_char) -> u32 {
    let result = catch_unwind(AssertUnwindSafe(|| {
        if name.is_null() { return None; }
        let n = unsafe { CStr::from_ptr(name) }.to_str().ok()?;
        let reg = get_geo_registry().lock().unwrap();
        let geo = reg.get(&handle)?;
        let a = geo_attr(geo, class, n)?;
        Some(attr_len(a))
    }));
    result.unwrap_or(None).unwrap_or(0)
}
...
#[no_mangle]
pub extern "C" fn cunning_geo_get_attr_type(handle: Handle, class: GeoAttrClass, name: *const c_char) -> GeoAttrType {
    let result = catch_unwind(AssertUnwindSafe(|| {
        if name.is_null() { return None; }
        let n = unsafe { CStr::from_ptr(name) }.to_str().ok()?;
        let reg = get_geo_registry().lock().unwrap();
        let geo = reg.get(&handle)?;
        let a = geo_attr(geo, class, n)?;
        if a.as_slice::<f32>().is_some() || a.as_paged::<f32>().is_some() { return Some(GeoAttrType::F32); }
        if a.as_slice::<Vec2>().is_some() || a.as_paged::<Vec2>().is_some() { return Some(GeoAttrType::Vec2); }
        if a.as_slice::<Vec3>().is_some() || a.as_paged::<Vec3>().is_some() { return Some(GeoAttrType::Vec3); }
        if a.as_slice::<Vec4>().is_some() || a.as_paged::<Vec4>().is_some() { return Some(GeoAttrType::Vec4); }
        if a.as_slice::<i32>().is_some() || a.as_paged::<i32>().is_some() { return Some(GeoAttrType::I32); }
        if a.as_slice::<IVec2>().is_some() || a.as_paged::<IVec2>().is_some() { return Some(GeoAttrType::IVec2); }
        if a.as_slice::<bool>().is_some() || a.as_paged::<bool>().is_some() { return Some(GeoAttrType::BoolU8); }
        Some(GeoAttrType::Unknown)
    }));
    result.unwrap_or(None).unwrap_or(GeoAttrType::Unknown)
}

fn pack_f32<T: Clone + Send + Sync + 'static>(a: &Attribute, dim: usize, mut f: impl FnMut(&T) -> [f32; 4]) -> Option<Vec<f32>> {
    let mut out: Vec<f32> = Vec::new();
    if let Some(s) = a.as_slice::<T>() {
        out.reserve(s.len() * dim);
        for v in s { let x = f(v); out.extend_from_slice(&x[..dim]); }
        return Some(out);
    }
    if let Some(pb) = a.as_paged::<T>() {
        out.reserve(pb.len() * dim);
        for v in pb.iter() { let x = f(&v); out.extend_from_slice(&x[..dim]); }
        return Some(out);
    }
    None
}
...
#[no_mangle]
pub extern "C" fn cunning_geo_copy_attr_f32(handle: Handle, class: GeoAttrClass, name: *const c_char, out_ptr: *mut f32, out_cap_f32: u32) -> u32 {
    let result = catch_unwind(AssertUnwindSafe(|| {
        if name.is_null() || out_ptr.is_null() || out_cap_f32 == 0 { return None; }
        let n = unsafe { CStr::from_ptr(name) }.to_str().ok()?;
        let reg = get_geo_registry().lock().unwrap();
        let geo = reg.get(&handle)?;
        let a = geo_attr(geo, class, n)?;
        let (dim, buf) = if let Some(v) = pack_f32::<f32>(a, 1, |x| [*x, 0.0, 0.0, 0.0]) { (1usize, v) }
        else if let Some(v) = pack_f32::<Vec2>(a, 2, |p| [p.x, p.y, 0.0, 0.0]) { (2usize, v) }
        else if let Some(v) = pack_f32::<Vec3>(a, 3, |p| [p.x, p.y, p.z, 0.0]) { (3usize, v) }
        else if let Some(v) = pack_f32::<Vec4>(a, 4, |p| [p.x, p.y, p.z, p.w]) { (4usize, v) }
        else { return Some(0u32); };
        let want = buf.len().min(out_cap_f32 as usize);
        unsafe { std::ptr::copy_nonoverlapping(buf.as_ptr(), out_ptr, want); }
        let _ = dim; // kept for clarity; caller knows by querying type
        Some(want as u32)
    }));
    result.unwrap_or(None).unwrap_or(0)
}
...
#[no_mangle]
pub extern "C" fn cunning_geo_copy_attr_i32(handle: Handle, class: GeoAttrClass, name: *const c_char, out_ptr: *mut i32, out_cap_i32: u32) -> u32 {
    let result = catch_unwind(AssertUnwindSafe(|| {
        if name.is_null() || out_ptr.is_null() || out_cap_i32 == 0 { return None; }
        let n = unsafe { CStr::from_ptr(name) }.to_str().ok()?;
        let reg = get_geo_registry().lock().unwrap();
        let geo = reg.get(&handle)?;
        let a = geo_attr(geo, class, n)?;
        let mut buf: Vec<i32> = Vec::new();
        if let Some(s) = a.as_slice::<i32>() { buf.extend_from_slice(s); }
        else if let Some(pb) = a.as_paged::<i32>() { buf.extend(pb.iter()); }
        else if let Some(s) = a.as_slice::<IVec2>() { for p in s { buf.push(p.x); buf.push(p.y); } }
        else if let Some(pb) = a.as_paged::<IVec2>() { for p in pb.iter() { buf.push(p.x); buf.push(p.y); } }
        else { return Some(0u32); }
        let want = buf.len().min(out_cap_i32 as usize);
        unsafe { std::ptr::copy_nonoverlapping(buf.as_ptr(), out_ptr, want); }
        Some(want as u32)
    }));
    result.unwrap_or(None).unwrap_or(0)
...
#[no_mangle]
pub extern "C" fn cunning_geo_copy_attr_u8(handle: Handle, class: GeoAttrClass, name: *const c_char, out_ptr: *mut u8, out_cap_u8: u32) -> u32 {
    let result = catch_unwind(AssertUnwindSafe(|| {
        if name.is_null() || out_ptr.is_null() || out_cap_u8 == 0 { return None; }
        let n = unsafe { CStr::from_ptr(name) }.to_str().ok()?;
        let reg = get_geo_registry().lock().unwrap();
        let geo = reg.get(&handle)?;
        let a = geo_attr(geo, class, n)?;
        let mut buf: Vec<u8> = Vec::new();
        if let Some(s) = a.as_slice::<bool>() { buf.reserve(s.len()); for b in s { buf.push(if *b { 1 } else { 0 }); } }
        else if let Some(pb) = a.as_paged::<bool>() { buf.reserve(pb.len()); for b in pb.iter() { buf.push(if b { 1 } else { 0 }); } }
        else { return Some(0u32); }
        let want = buf.len().min(out_cap_u8 as usize);
        unsafe { std::ptr::copy_nonoverlapping(buf.as_ptr(), out_ptr, want); }
        Some(want as u32)
    }));
    result.unwrap_or(None).unwrap_or(0)
